...
- **Package the QGIS plugin:** the `Makefile` targets (`deploy`, `zip`, `package`) are for QGIS plugin packaging (`pb_tool`-style); `compile` builds `resources.py` from `resources.qrc` via `pyrcc5`. Release ZIPs are produced automatically on git **tag** push (`on-push-tags.yml`), which inlines the tag into `metadata.txt`.
