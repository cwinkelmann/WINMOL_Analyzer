...
- **Python / deps:** target 3.9–3.11. **The runtime is TensorFlow-free** — models are ONNX and run through `onnxruntime`; TensorFlow is only needed by `requirements/convert.txt`, the dev-only HDF5→ONNX script. Install `requirements/cpu.txt` (the geo stack + `onnxruntime` + `psutil`); on an NVIDIA machine install `requirements/gpu.txt` instead — never both, since `onnxruntime` and `onnxruntime-gpu` provide the same module. The algebra is one shared runtime-free fragment plus one runtime: `cpu = core + onnxruntime`, `gpu = core + onnxruntime-gpu`, `notebook = cpu + plots`, `ci = cpu + pytest`. CI installs what ships — `ci.txt` hangs off `cpu.txt` so the suite runs in the user's environment, and `tests/test_ci_parity.py` fails if they diverge. Canonical table, and how the CLI/plugin/Docker/CI environments relate: `requirements/README.md`.
